<template>
  <div class="flex items-center gap-3 p-4 rounded-2xl bg-slate-900/45 border border-white/5 backdrop-blur-xl">
    <div class="relative">
      <svg
      ...
      <div
        v-if="isActive"
        class="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-orange-500 border-2 border-slate-900 animate-pulse"
      />
    </div>
    <div>
      <div class="text-2xl font-bold text-orange-400">
        {{ streakCount }}
        <span class="text-sm font-normal text-slate-400">ngày</span>
      </div>
