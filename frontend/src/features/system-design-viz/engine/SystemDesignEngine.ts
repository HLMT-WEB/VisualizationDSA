...
/**
 * Core routing engine for distributed system simulation.
 * Manages nodes, links, Round-Robin load balancing, failover, and packet lifecycle.
 */
export class SystemDesignEngine {
  ...
  public registerLink(link: NetworkLink): void {
    const exists = this.links.some((l) => l.linkId === link.linkId);
    if (!exists) {
  ...
  public setNodeStatus(nodeId: string, status: SystemNode['status']): void {
  ...
  /**
   * Route a packet from Load Balancer to healthy servers using Round-Robin.
   * Returns null if no healthy servers are available (all failed).
   */
  public routeRequestFromLB(lbId: string, packetColor: string): NetworkPacket | null {
    ...
    const targetServer =
      connectedHealthyServers[this.lbActiveIndex % connectedHealthyServers.length];
  ...
  /**
   * Create a direct packet between any two nodes (for DB write, cache, etc.)
   */
  public createDirectPacket(
    sourceId: string,
    targetId: string,
    packetColor: string,
  ): NetworkPacket | null {
  ...
  /**
   * Advance all in-transit packets by deltaTime. Remove arrived packets (GC).
   */
